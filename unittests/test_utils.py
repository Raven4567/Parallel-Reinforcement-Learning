...
class TestUtils(unittest.TestCase):
    ...
    def test_number_of_active_environments(self):
        AsyncTools.utils.number_of_active_environments(np.random.choice([False, True], 4))

    def test_range_of_active_environments(self):
        AsyncTools.utils.range_of_active_environments(np.random.choice([False, True], 4))
    
    def test_inactive_states_dropout(self):
        AsyncTools.utils.inactive_states_dropout(np.random.randn(4, 4), np.random.choice([False, True], 4))
    
    def test_buffer_append(self):
        AsyncTools.utils.buffer_append(
            buffer=AsyncTools.AsyncPPO.VecMemory(num_envs=4),
    ...
    def test_update_active_environments_list(self):
        ...
        AsyncTools.utils.update_active_environments_list(random_enviroments_activity, np.random.choice([False, True], size=4-np.sum(random_enviroments_activity)))
    ...
    def test_buffer_to_target_buffer_transfer(self):
        ...
        AsyncTools.utils.buffer_to_target_buffer_transfer(
            buffer,
